...
class Program
{
    static async Task SetAilerons(SimConnectClient client, double normalized)
    {
        normalized = Math.Max(-1.0, Math.Min(1.0, normalized));

        // SimVar aileron en écriture
        await client.SimVars.SetAsync(
            "AILERON POSITION",
            "position",
            normalized
        );
